...
; RECOMENDAMOS TAMBÉM COMENTAR O SIGNIFICADO DE SEUS ESTADOS (0 E 1)	
	...
	#DEFINE THETA_FLAG	    LATA,1	    ;PULSO DE DEFINIÇÃO DA INCLINAÇÃO DO SERVO MOTOR1 
	#DEFINE PHI_FLAG	    LATA,2	    ;PULSO DE DEFINIÇÃO DA INCLINAÇÃO DO SERVO MOTOR2
...
; INTERRUPÇÕES
	...
	BTFSS	AJUSTE_THETA_FLAG   ;TESTA SE O AJUSTE DO ANGULO THETA JÁ FOI FEITO
	GOTO	INICIA_CONVERSAO    ;SE NÃO INICIA A CONVERSÃO
	BSF	ANGULO_ATIVO_FLAG   ;ALTERA O ANGULO ATIVO PARA PHI	
	BTFSC	AJUSTE_PHI_FLAG	    ;TESTA SE O PHI JÁ FOI AJUSTADO
	GOTO	MOTORES_POSICIONADOS;SE PHI JÁ AJUSTADO, FINALIZA O CICLO DE AJUSTE
INICIA_CONVERSAO
...
CONVERSAO_THETA
	...
	CPFSLT	V_MAX_THETA	    ;V_MAX_THETA < WREG(VALOR CONVERTIDO) ---> SE SIM, ATUALIZA O VALOR MÁXIMO E SEU ÂNGULO
...
CONVERSAO_PHI
	MOVF	ADRESH,0	    ;MOVE O VALOR DA CONVERSAO PARA WREG
	CPFSLT	V_MAX_PHI	    ;V_MAX_PHI < WREG(VALOR CONVERTIDO) ---> SE SIM, ATUALIZA O VALOR MÁXIMO E SEU ÂNGULO 
	GOTO	ANGULO_PHI	    ;SE NÃO, DESVIO PARA O TRATAMENTO DO ANGULO PHI
...
INCREMENTA_THETA
	...
	MOVFF	MELHOR_THETA,THETA  ;SE THETA FOR 181, ENTAO THETA RETORNA AO MELHOR ANGULO
...
INCREMENTA_PHI
	...
	MOVFF	MELHOR_PHI,PHI	    ;SE PHI FOR 181, ENTAO PHI RETORNA AO MELHOR ANGULO
	BSF	AJUSTE_PHI_FLAG	    ;SINALIZA QUE ACABOU O AJUSTE DA POSICAO PHI
	GOTO	END_INT
...
MAIN
	...
	BTFSC	FIM_CICLO_FLAG	    ;TESTA SE O POSICIONAMENTO JÁ FOI REALIZADO
	GOTO	AGORA_PERMANECE	    ;SE FIM_CICLO = 1, ENTÃO PERMANECE NAS MELHORES POSIÇÕES
	...
	BCF	THETA_FLAG	    ;ATÉ O PERÍODO TERMINAR, RA1 = 0	
	GOTO	TERMINOU_PERIODO    ;ESPERA O PERÍODO DE 20 MILISEGUNDOS ACABAR
	
ALTERA_PHI
	BSF	PHI_FLAG	    ;RA2 EQUIVALE AO SINAL DO SERVO MOTOR 2, PHI
	CALL	DELAY_600U	    ;PERMANECE POR 600USEGUNDOS
	MOVFF	PHI,CONTADOR_DELAY  ;MAIS PHI*10USEGUNDOS EM NÍVEL ALTO
	CALL	DELAY_AUXILIAR	    ;CONTADOR É INICIALIZADO COM O VALOR DO ÂNGULO
	BCF	PHI_FLAG	    ;ATÉ O PERÍODO TERMINAR, RA2 = 0	
	GOTO	TERMINOU_PERIODO
	
AGORA_PERMANECE
	BTG	ANGULO_ATIVO_FLAG   ;ALTERA ENTRE PHI E THETA A CADA NOVO CICLO
	;GOTO	TERMINOU_PERIODO    ;AFIM DE ESTABILIZAR OS ANGULOS PERPENDICULARES A FONTE DE LUZ
